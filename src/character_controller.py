...
class CharacterController:
    ...
    def __init__(self, root: tk.Tk, mascot_app: 'DesktopMascot', character_id: str, directory_name: str, is_flipped: bool, config: ConfigParser, position_side: str, input_history_manager: InputHistoryManager):
        ...
        try:
            ...
            transparency_mode = self.char_config.get(info_section, 'TRANSPARENCY_MODE', fallback=mascot_app.default_transparency_mode)
            self.transparency_mode = transparency_mode.lower()
            ...
            self.transparency_mode = mascot_app.default_transparency_mode

        if self.transparency_mode not in ('color_key', 'alpha'):
            self.transparency_mode = 'color_key'
        ...
        self.heart_transparency_mode = 'color_key' # デフォルト値
    ...
    def _load_heart_ui_config(self):
        ...
        if not self.char_config.has_section(section_name):
            self.heart_transparency_mode = 'color_key' # セクションがない場合はデフォルト
        ...
        # 新しく TRANSPARENCY_MODE を読み込む
        mode = self.char_config.get(section_name, 'TRANSPARENCY_MODE', fallback='color_key').lower()
        if mode not in ('color_key', 'alpha'):
            mode = 'color_key'

        self.heart_transparency_mode = mode

        print(f"[{self.name}] ハート専用UI設定を読み込みました: "
              f"透過色={self.heart_transparent_color}, 縁色={self.heart_edge_color}, 透過モード={self.heart_transparency_mode}")
    ...
    def reload_character_data(self):
        ...
        try:
            ...
            transparency_mode = self.char_config.get('INFO', 'TRANSPARENCY_MODE', fallback=self.mascot_app.default_transparency_mode)
            self.transparency_mode = transparency_mode.lower()

            if self.transparency_mode not in ('color_key', 'alpha'):
                self.transparency_mode = 'color_key'

            if self.transparency_mode == 'alpha' and os.name != 'nt':
                print(f"警告: TRANSPARENCY_MODE=alpha は現在のプラットフォームではサポートされていないため、color_key にフォールバックします。")
                self.transparency_mode = 'color_key'
    ...
    def resize(self, new_window_width: int):
        """UIにウィンドウサイズ変更を指示する"""
        if self.ui and self.ui.winfo_exists():
            self.ui.resize(new_window_width)

    def update_nameplate_visibility(self, is_visible: bool):
        """UIに名前欄の表示/非表示を指示する"""
        if self.ui and self.ui.winfo_exists():
            self.ui.update_nameplate_visibility(is_visible)

    def update_nameplate_size(self, size_key: str):
        """UIに名前欄のサイズ変更を指示する"""
        if self.ui and self.ui.winfo_exists():
            self.ui.update_nameplate_size(size_key)