...
class MemoryManager:
    ...
    def __init__(self, character_dir_path: str, config: ConfigParser):
        """
        MemoryManagerを初期化します。

        Args:
            character_dir_path (str): この記憶が属するキャラクターのディレクトリパス。
            config (ConfigParser): アプリケーション全体の設定情報。
        """
        self.file_path = os.path.join(character_dir_path, self.LTM_FILE_NAME)
        ...
        try:
            ...
            if self.ltm_history_limit <= 0:
                print(f"警告: LONG_TERM_MEMORY_LIMIT の値 ({self.ltm_history_limit}) が不正です。デフォルト値(20)を使用します。")
        ...
        try:
            ...
            if self.ltm_prompt_limit <= 0:
                print(f"警告: LONG_TERM_MEMORY_PROMPT_LIMIT の値 ({self.ltm_prompt_limit}) が不正です。デフォルト値(10)を使用します。")
