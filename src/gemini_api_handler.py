...
class GeminiAPIHandler:
    ...
    def list_available_models(api_key: str) -> dict:
        ...
        # 2. 形式がGoogle AIのAPIキーとして妥当か（文字数、プレフィックス、文字種）
        # Google AI APIキーは通常 "AIza" で始まり、英数字とハイフン/アンダースコアで構成される39文字
        print(api_key)
        api_key_pattern = re.compile(r"^AIza[0-9A-Za-z_-]{35}$")
        if not api_key_pattern.match(api_key):
            print(f"APIキーが不正な形式です。キー: {api_key[:10]}...")
            return {'status': 'auth_error', 'models': None, 'error_message': 'APIキーの形式が無効です。文字数やプレフィックスを確認してください。'}
