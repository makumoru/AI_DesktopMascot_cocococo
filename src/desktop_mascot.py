...
from src.startup_selector import StartupCharacterSelector
...
class DesktopMascot:
    ...
    def __init__(self, app_root_dir: str, current_version: str):
        ...
        # --- config.iniに存在しない項目があれば、デフォルト値で追記する ---
        ...
        # 各設定項目をチェックし、なければデフォルト値を追加
        default_settings = {
            'WINDOW_WIDTH_RATIO': ('; ウィンドウ幅の画面サイズに対する比率 (デフォルト: 0.2)\n', '0.2'),
            'STARTUP_MODE': ('; 起動時のモード(fixed(通常),select(起動時に選択),random(ランダム選出))\n', 'fixed'),
            'SPECIAL_TOPIC_BASE_WEIGHT': ('\n; --- 自動発話の専用話題に関する設定 ---\n; キャラクター固有の話題が選ばれる確率の基本値 (%) (デフォルト: 10)\n', '10'),
            'SPECIAL_TOPIC_PER_ITEM_WEIGHT': ('; キャラクター固有の話題が1つ増えるごとに加算される確率 (%) (デフォルト: 1)\n', '1'),
            'SPECIAL_TOPIC_MAX_WEIGHT': ('; キャラクター固有の話題が選ばれる確率の上限値 (%) (デフォルト: 70)\n', '70'),
            'SHOW_NAMEPLATE': ('\n; --- 名前欄（ネームプレート）に関する設定 ---\n; 名前欄を表示するかどうか (True/False)\n', 'True'),
            'NAMEPLATE_SIZE': ('; 名前欄のサイズ (small/medium/large)\n', 'medium'),
        }

        for key, (comment, value) in default_settings.items():
            if not self.config.has_option('UI', key):
                print(f"config.iniに '{key}' が見つからないため、デフォルト値を追加します...")
                # コメントと設定値を文字列として結合
                setting_str = f"{comment}{key.upper()} = {value}\n"
                
                # [UI]セクションの末尾に追記するために、一度ファイルを読む（やや非効率だが確実）
                try:
                    ...
                    for i, line in enumerate(lines):
                        if line.strip().lower() == '[ui]':
                            ui_section_index = i
                            break
                    
                    # [UI]セクションの末尾を探す
                    insert_pos = len(lines)
                    if ui_section_index != -1:
                        insert_pos = ui_section_index + 1
                        while insert_pos < len(lines) and not lines[insert_pos].strip().startswith('['):
                            insert_pos += 1
                    
                    lines.insert(insert_pos, setting_str)
                    ...
                    print(f"config.iniへの '{key}' の追記中にエラー: {e}")

        # 設定ファイルが更新された場合は、再読み込み
        if config_updated:
            self.config.read('config.ini', encoding='utf-8-sig')
            print("config.iniを更新・再読み込みしました。")
        ...
        self.pos_config_path = 'savedata/position.ini'


        # 推奨通知の無視リストのパス
        self.recommendation_log_path = 'savedata/recommendation_log.ini'
        ...
        # 起動キャラクターの準備がキャンセルされたら、ここで処理を終了する
        if not self._prepare_character_launch():
        ...
        # EventManagerのインスタンス化 (キャラクターの初期化後)
        ...
        self.is_event_running = False # イベント実行中フラグ
        ...
        self._post_event_callback = None
    ...
    def _load_config_values(self):
        ...
        # 専用話題の抽選設定を読み込む
        self.special_topic_base_weight = self.config.getint('UI', 'SPECIAL_TOPIC_BASE_WEIGHT', fallback=10)
        self.special_topic_per_item_weight = self.config.getint('UI', 'SPECIAL_TOPIC_PER_ITEM_WEIGHT', fallback=1)
        self.special_topic_max_weight = self.config.getint('UI', 'SPECIAL_TOPIC_MAX_WEIGHT', fallback=70)
        ...
        self.default_transparency_mode = self.config.get('UI', 'TRANSPARENCY_MODE', fallback='color_key').lower()
        if self.default_transparency_mode not in ('color_key', 'alpha'):
            self.default_transparency_mode = 'color_key'
        ...
        self.is_nameplate_visible = tk.BooleanVar(value=self.config.getboolean('UI', 'SHOW_NAMEPLATE', fallback=True))
        self.nameplate_size_setting = tk.StringVar(value=self.config.get('UI', 'NAMEPLATE_SIZE', fallback='medium'))
        
        # ウィンドウサイズ設定用のStringVar
        current_ratio = self.config.getfloat('UI', 'WINDOW_WIDTH_RATIO', fallback=0.2)
        size_key = "medium" # デフォルト
        if current_ratio <= 0.15: size_key = "small"
        elif current_ratio >= 0.25: size_key = "large"
        self.window_size_setting = tk.StringVar(value=size_key)
        ...
        # --- 設定変更を検知し、タスクトレイの表示を同期させるためのコールバックを登録 ---
        self.is_always_on_top.trace_add("write", self._toggle_always_on_top)
        self.is_sound_enabled.trace_add("write", self._toggle_mute)
        self.is_auto_speech_enabled.trace_add("write", self._toggle_auto_speech)
        self.is_schedule_enabled.trace_add("write", self._toggle_schedules)
        self.is_pro_mode.trace_add("write", self._toggle_pro_mode)
        self.is_screenshot_mode.trace_add("write", self._toggle_screenshot_mode)
        self.is_nameplate_visible.trace_add("write", self._toggle_nameplate_visibility)
        self.nameplate_size_setting.trace_add("write", self._set_nameplate_size)
        self.window_size_setting.trace_add("write", self._set_window_size)
    ...
    def change_character(self, target_char_id, new_char_dir_name):
        ...
        if self.is_processing_lock.acquire(blocking=False):
            ...
            def swap_and_greet():
                ...
                # EventManagerから古いキャラクターのデータをアンロード
                self.event_manager.unload_data_for_character(old_char)
                ...
                def finalize_and_greet():
                    ...
                    # EventManagerに新しいキャラクターのデータをロード
                    self.event_manager.load_character_data(new_char)
                    self.event_manager.load_events_for_character(new_char)
                    ...
                    # 2d. 挨拶シーケンスの次のステップへ
                    # 新キャラクターに即時実行可能なイベントがあるかチェック
                    ready_event = self.event_manager.find_ready_event_for_character(new_char)
                    if ready_event:
                        print(f"キャラクター交代後、イベント '{ready_event.get('id')}' の条件を満たしているため、挨拶をスキップしてイベントを開始します。")
                        # イベント終了後に相方の反応チェックへ進むようにコールバックを設定
                        self._post_event_callback = lambda: react_to_change(new_char, new_char.partner)
                        self.start_event(new_char, ready_event)
                    else:
                        # イベントがない場合は、通常の挨拶シーケンスへ
    ...
    def _prepare_character_launch(self):
        """
        config.iniの設定に基づき、起動するキャラクターを決定し、
        position.iniを動的に更新する。
        Returns:
            bool: 起動を続行する場合はTrue、中止する場合はFalse
        """
        startup_mode = self.config.get('UI', 'STARTUP_MODE', fallback='fixed')

        if startup_mode == 'fixed':
            # 従来通り、既存のposition.iniを読み込む
            if os.path.exists(self.pos_config_path):
                self.pos_config.read(self.pos_config_path, encoding='utf-8')
            else:
                # position.iniがない場合は、config.iniからフォールバック
                print(f"警告: {self.pos_config_path} が見つかりません。config.iniの設定で起動します。")
                self.pos_config = self.config
            return True

        selected_dirs = None
        if startup_mode == 'select':
            # 1. ダイアログ表示のために、非表示になっている親ウィンドウを一時的に表示可能にする
            print("desktop_mascot.py > _prepare_character_launch > self.root.deiconify()")
            self.root.deiconify()
            # 2. ただし、ユーザーに見えないように画面の外へ移動させる
            self.root.geometry("+9999+9999")
            
            # 3. キャラクター選択ダイアログを生成・表示
            selector = StartupCharacterSelector(self.root, self.project_manager, self.theme_manager, self.font_normal, self.font_title, self.padding_normal)
            selected_dirs = selector.selected_characters
            
            # 4. ダイアログが閉じたら、再び親ウィンドウを非表示に戻す
            self.root.withdraw()
        
        elif startup_mode == 'random':
            all_chars = self.project_manager.list_projects()
            if not all_chars:
                messagebox.showerror("起動エラー", "charactersフォルダにキャラクターが見つかりません。")
                return False
            selected_dirs = [random.choice(all_chars)]

        if selected_dirs: # 選択またはランダム選出が成功した場合
            self._update_position_config_dynamically(selected_dirs)
            return True
        else: # 選択がキャンセルされた場合
            print("キャラクター選択がキャンセルされたため、アプリケーションを終了します。")
            return False

    def _update_position_config_dynamically(self, char_dirs: list):
        """
        指定されたキャラクターディレクトリリストに基づいてposition.iniを上書きする。
        """
        print(f"position.iniを動的に更新します: {char_dirs}")
        new_pos_config = ConfigParser()

        # 1人目
        if len(char_dirs) >= 1:
            new_pos_config['CHARACTER_1'] = {
                'DIRECTORY': char_dirs[0],
                'POSITION_SIDE': 'right',
                'IS_FLIPPED': 'False'
            }
        ...
        try:
            # savedataフォルダがなければ作成
            os.makedirs(os.path.dirname(self.pos_config_path), exist_ok=True)
            with open(self.pos_config_path, 'w', encoding='utf-8') as f:
                new_pos_config.write(f)
            # 内部で保持している設定オブジェクトも更新
            self.pos_config = new_pos_config
        except Exception as e:
            messagebox.showerror("エラー", f"position.iniの書き込みに失敗しました:\n{e}")

    def _setup_characters(self):
        """キャラクターのインスタンスを生成し、連携を設定します。position.iniを使用します。"""
        self.characters = []
        self.char1, self.char2 = None, None
        self.is_shutting_down = False

        main_config = self.pos_config
        
        print("キャラクター設定の読み込み元: position.ini (動的生成含む)")
        
        # --- キャラクター1の読み込み ---
        try:
            char1_dir = main_config.get('CHARACTER_1', 'DIRECTORY')
            char1_is_flipped = main_config.getboolean('CHARACTER_1', 'IS_FLIPPED', fallback=False)
        ...
        if self.is_char2_enabled:
            try:
                ...
                char2_is_flipped = main_config.getboolean('CHARACTER_2', 'IS_FLIPPED', fallback=True)
    ...
    def _create_tools_config_for_character(self, character):
        ...
        function_declarations = [
            ...
            {"name": "change_costume", "description": "キャラクターの衣装を変更します。", "parameters": {"type": "object", "properties": {"costume_id": {"type": "string", "description": "変更したい衣装のID。"}}, "required": ["costume_id"]}},
            {
                ...
                "description": "直前の会話を評価し、「知識」として長期記憶に保存すべきか判断する。これは「恒久的に保存するべき知識」です。ユーザーとの関係を深める重要な情報や、キャラクターの行動に影響を与える事実などが対象となります。既にある長期記憶と重複する内容、単なる会話内容やアクション内容や挨拶や相槌は保存しないこと。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "is_important": {"type": "boolean", "description": "この会話は「知識」として長期的に記憶する価値があるか。"},
                        "importance_score": {"type": "integer", "description": "知識を記憶する場合の重要度 (1-100)。数値が高いほど重要。"},
                        "summary": {"type": "string", "description": "記憶すべき知識を、誰が読んでも理解できるように簡潔に要約したテキスト。"}
    ...
    def add_character(self, new_char_dir_name):
        ...
        with self.is_processing_lock:
            ...
            # EventManagerに新しいキャラクターのデータをロード
            self.event_manager.load_character_data(self.char2)
            self.event_manager.load_events_for_character(self.char2)
            ...
            # 追加したキャラクターの初期配置を確定させた上で、表示要素を更新する
            self.char2.ui.finalize_initial_position()
            self.root.after(100, self.char2.ui.update_info_display)

            # 6. 新しく登場したキャラクターに挨拶させるか、イベントを開始するか
            self._update_all_character_maps() 

            ready_event = self.event_manager.find_ready_event_for_character(self.char2)
            if ready_event:
                print(f"キャラクター追加後、イベント '{ready_event.get('id')}' の条件を満たしているため、挨拶をスキップしてイベントを開始します。")
                # イベント終了後はロック解放とクールタイムリセットのみ行う
                self._post_event_callback = lambda: (
                    self.reset_cool_time(),
                    self.is_processing_lock.release() if self.is_processing_lock.locked() else None
                )
                self.start_event(self.char2, ready_event)
            else:
                # イベントがなければ通常の挨拶
    ...
    def dismiss_character(self, character_to_dismiss):
        ...
        with self.is_processing_lock:
            # EventManagerから退去するキャラクターのデータをアンロード
            self.event_manager.unload_data_for_character(character_to_dismiss)
    ...
    def reload_all_settings(self):
        """
        アプリケーションを再起動せずに、すべての設定ファイルとキャラクター情報を再読み込みします。
        """
        print("--- 全設定の再読み込みを開始します ---")
        if self.is_processing_lock.locked():
            print("他の処理が実行中のため、再読み込みをスキップしました。")
            # ユーザーにフィードバックを返す
            if self.char1:
                self.char1.ui.output_box.set_text("今は他のことを考えてて、設定を読めないみたい…")
            return

        try:
            # 1. グローバル設定(config.ini)の再読み込み
            self.config.read('config.ini', encoding='utf-8')

            # 2. グローバル設定に依存するコアサービスを更新
            self._load_config_values() # APIキーやモデル名などを再読み込み
            self.gemini_handler = GeminiAPIHandler(self.config) # APIハンドラを新しい設定で再生成
            self.global_voice_engine_manager.global_config = self.config # 音声エンジンマネージャに新しい設定を渡す
            self.theme_manager.load_theme() # カラーテーマを再読み込み

            # 3. 各キャラクターに設定の再読み込みを指示
            for char in self.characters:
                if char:
                    char.reload_character_data()

            # 4. スケジュールとイベントの再読み込み
            self.schedule_manager.schedules = self.schedule_manager._load_schedules()
            self.event_manager.load_all_character_data() # イベント定義と進行状況を再読み込み

            # 5. 全UIのテーマを再適用
            self._reload_ui_theme()

            # 6. ユーザーに完了を通知
            print("--- 全設定の再読み込みが完了しました ---")
            if self.char1:
                self.char1.ui.output_box.set_text("設定を更新したよ！")
    ...
    def exit_app(self):
        ...
        # このコールバックは、音声再生完了後、または終了ボタンクリック後に呼ばれる
        def on_final_action():
        ...
        if visible_characters:
            # post_speech_callback を on_final_action に設定
            self._post_speech_callback = on_final_action
            speaker = random.choice(visible_characters)
            prompt = "アプリケーションを終了します。ユーザーに短いお別れの挨拶をしてください。"
            
            # 挨拶リクエストの前にメッセージを表示
            # UIの更新はメインスレッドで行う必要があるため、after(0, ...) を使用する
            self.root.after(0, speaker.ui.output_box.set_text, "(終了処理中)")
    ...
    def toggle_visibility(self):
        """キャラクターウィンドウと関連ウィンドウの表示/非表示を切り替えます。"""
        ...
        for char in self.characters:
            if char and char.ui.winfo_exists():
                if is_visible:
                    ...
                    # ★ここから追加
                    if char.ui.overlay_window.winfo_exists():
                        char.ui.overlay_window.withdraw()
                    # ★ここまで追加
                else:
                    # --- 再表示処理 ---
                    print("desktop_mascot.py > toggle_visibility > char.ui.deiconify()")
                    char.ui.deiconify()
                    if char.ui.heart_window.winfo_exists() and char.get_current_heart_image_filename():
                        print("desktop_mascot.py > toggle_visibility > char.ui.heart_window.deiconify()")
                        char.ui.heart_window.deiconify()
                    # ★ここから追加
                    # オーバーレイは通常非表示。イベントで必要になったら表示される。
                    # なのでここでは何もしない。
                    # ★ここまで追加
    ...
    def bring_all_to_front(self):
        ...
        if self.char1 and self.char1.ui.winfo_exists():
            self.char1.ui.lift_window()
        if self.char2 and self.char2.ui.winfo_exists():
            self.char2.ui.lift_window()
    ...
    def _bring_ui_to_front(self, char_ui):
        ...
        def collect_window(window_like):
            if not window_like:
                return
            try:
                if window_like.winfo_exists():
                    windows_to_raise.append(window_like)
            except tk.TclError:
                pass

        collect_window(char_ui)
        collect_window(getattr(char_ui, "name_window", None))
        collect_window(getattr(char_ui, "heart_window", None))
        collect_window(getattr(char_ui, "overlay_window", None))

        alpha_overlay = getattr(char_ui, "alpha_overlay", None)
        if alpha_overlay:
            collect_window(getattr(alpha_overlay, "window", None))

        heart_alpha_overlay = getattr(char_ui, "heart_alpha_overlay", None)
        if heart_alpha_overlay:
            collect_window(getattr(heart_alpha_overlay, "window", None))

        if not windows_to_raise:
            return

        for window in windows_to_raise:
            try:
        ...
        try:
            char_ui.after(200, reset_topmost)
        except tk.TclError:
            self.root.after(200, reset_topmost)
    ...
    def show_context_menu(self, event, target_char):
        """右クリックイベントをUIManagerに中継します。"""
        self.ui_manager.show_context_menu(event, target_char)
    ...
    def _apply_initial_settings(self):
        """起動時にconfigから読み込んだ設定をUIに適用し、初回位置を確定させます。"""
        # is_always_on_topなどの設定を適用 (traceコールバックが自動で実行される)
        self.is_always_on_top.set(self.is_always_on_top.get())
    ...
    def _update_tray_menu_safely(self):
        """タスクトレイメニューをメインスレッドで安全に更新します。"""
        if self.tray_icon:
            self.root.after(0, self.tray_icon.update_menu)
    ...
    def _toggle_always_on_top(self, *args):
        def safe_update():
            ...
            for char in self.characters:
                ...
                ui = getattr(char, "ui", None)
                if not ui:
                    continue

                try:
                    ui_exists = ui.winfo_exists()
                except tk.TclError:
                    continue
                ...
                def apply_topmost(window_like):
                    if not window_like:
                        return
                    try:
                        if window_like.winfo_exists():
                            window_like.wm_attributes("-topmost", new_state)
                    except tk.TclError:
                        pass

                apply_topmost(ui)
                apply_topmost(getattr(ui, "name_window", None))
                apply_topmost(getattr(ui, "heart_window", None))
                apply_topmost(getattr(ui, "overlay_window", None))

                alpha_overlay = getattr(ui, "alpha_overlay", None)
                if alpha_overlay:
                    apply_topmost(getattr(alpha_overlay, "window", None))

                heart_alpha_overlay = getattr(ui, "heart_alpha_overlay", None)
                if heart_alpha_overlay:
                    apply_topmost(getattr(heart_alpha_overlay, "window", None))
    ...
    def _toggle_auto_speech(self, *args):
        def safe_update():
            self._update_config_file('UI', 'ENABLE_AUTO_SPEECH', self.is_auto_speech_enabled.get())
            self._update_tray_menu_safely()
        self.root.after(0, safe_update)

    def _toggle_schedules(self, *args):
        def safe_update():
            self._update_config_file('UI', 'ENABLE_SCHEDULES', self.is_schedule_enabled.get())
            self._update_tray_menu_safely()
        self.root.after(0, safe_update)

    def _toggle_pro_mode(self, *args):
        # この設定はconfigに保存しない揮発性のものです。
        self._update_tray_menu_safely()

    def _toggle_screenshot_mode(self, *args):
        # この設定はconfigに保存しない揮発性のものです。
        self._update_tray_menu_safely()

    def _toggle_nameplate_visibility(self, *args):
        """名前欄の表示/非表示設定が変更されたときのコールバック"""
        new_state = self.is_nameplate_visible.get()
        self._update_config_file('UI', 'SHOW_NAMEPLATE', new_state)
        for char in self.characters:
            if char:
                char.update_nameplate_visibility(new_state)
        # タスクトレイメニューも更新（将来的に追加する場合に備える）
        self._update_tray_menu_safely()

    def _set_nameplate_size(self, *args):
        """名前欄のサイズ設定が変更されたときのコールバック"""
        new_size = self.nameplate_size_setting.get()
        self._update_config_file('UI', 'NAMEPLATE_SIZE', new_size)
        for char in self.characters:
            if char:
                char.update_nameplate_size(new_size)
        # タスクトレイメニューも更新
        self._update_tray_menu_safely()

    def _set_window_size(self, *args):
        """ウィンドウサイズのプリセットが変更されたときのコールバック"""
        size_key = self.window_size_setting.get()
        
        # サイズキーと対応するwidth_ratioのマッピング
        size_map = {
            "small": 0.15,
            "medium": 0.20,
            "large": 0.25
        }
        new_ratio = size_map.get(size_key)
        if new_ratio is None:
            return # 不正なキーの場合は何もしない

        # 1. config.iniを更新
        self._update_config_file('UI', 'WINDOW_WIDTH_RATIO', new_ratio)
        
        # 2. アプリケーション内のウィンドウ幅を再計算
        screen_width = self.root.winfo_screenwidth()
        self.window_width = int(screen_width * new_ratio)
        
        # 3. 各キャラクターにサイズ変更を通知
        print(f"ウィンドウサイズを '{size_key}' ({self.window_width}px) に変更します。")
        for char in self.characters:
            if char:
                char.resize(self.window_width)
    ...
    def perform_synchronized_update(self, speaker, text, wav_data, emotion_jp, pass_turn):
        ...
        if self.is_shutting_down:
            ...
            # 終了シーケンス中は、以降のラリー処理などは行わないようにする
            pass_turn = False 

        if not self.is_always_on_top.get():
             self.bring_all_to_front()

        # 表示用のテキストを加工。「。」の直後に改行がない場合のみ改行を挿入する。
        display_text = re.sub(r'。(?!\n)', '。\n', text)
        speaker.ui.output_box.set_text(display_text or "...")
        # 感情変更時はウィンドウをリフトする
        speaker.ui.emotion_handler.update_image(emotion_jp, lift_ui=True)
        for char in self.characters:
        ...
        def on_finish_callback():
            ...
            if callback := self._post_speech_callback:
                ...
                # 終了シーケンス中で、かつ音声OFFの場合の特別処理
                is_sound_off = not wav_data or not self.is_sound_enabled.get()
                if self.is_shutting_down and is_sound_off:
                    # final_shutdownを直接呼ばず、UIに終了ボタンの表示を依頼する
                    # ボタンのコマンドとして、最終的なシャットダウン処理(callback)を渡す
                    speaker.ui.show_exit_button(callback)
                    return # ここで処理を中断し、ユーザーのクリックを待つ

                # 通常の処理（音声ONの終了時、または通常の発話時）
                callback()
                return
    ...
    def greet_on_startup(self):
        """起動時にキャラクターに挨拶をさせます。ただし、即時開始可能なイベントがあればそちらを優先します。"""
        if self.is_processing_lock.locked() or not self.characters: return
        
        speaker = random.choice(self.characters)

        # イベントチェック処理
        ready_event = self.event_manager.find_ready_event_for_character(speaker)
        if ready_event:
            print(f"起動時にイベント '{ready_event.get('id')}' の条件を満たしているため、起動挨拶をスキップしてイベントを開始します。")
            self.start_event(speaker, ready_event)
            # 挨拶の代わりに今日の終日イベントチェックを予約
            self.root.after(5000, lambda: self.trigger_daily_events_for_date(datetime.now().date()))
            return # イベントを開始したので、これ以降の挨拶処理は行わない

        # イベントがない場合のみ、通常の起動挨拶を実行
        with self.is_processing_lock:
    ...
    def trigger_auto_speech(self):
        ...
        try:
            ...
            # 話題のハイブリッド抽選
            # 1. 汎用話題を読み込む
            general_topics_path = self.config.get('UI', 'AUTO_SPEECH_TOPICS_FILE', fallback='savedata/topics.txt')
            general_topics = []
            if os.path.exists(general_topics_path):
                with open(general_topics_path, 'r', encoding='utf-8-sig') as f:
                    general_topics = [line.strip() for line in f if line.strip()]
            # 2. キャラクター専用話題を読み込む
            special_topics_path = os.path.join(speaker.character_dir, 'topics.txt')
            special_topics = []
            if os.path.exists(special_topics_path):
                with open(special_topics_path, 'r', encoding='utf-8-sig') as f:
                    special_topics = [line.strip() for line in f if line.strip()]
            # 3. 専用話題が選ばれる確率（重み）を計算
            special_topics_weight = 0
            if special_topics:
                # min(最大比率, 基本比率 + (専用話題の数 × 増加率))
                calculated_weight = self.special_topic_base_weight + (len(special_topics) * self.special_topic_per_item_weight)
                special_topics_weight = min(self.special_topic_max_weight, calculated_weight)
            # 4. どちらの話題プールから選ぶか抽選
            pools = ['general', 'special']
            weights = [100 - special_topics_weight, special_topics_weight]
            # どちらかのプールが空の場合は、必ずもう片方が選ばれるように重みを調整
            if not general_topics: weights = [0, 100]
            if not special_topics: weights = [100, 0]
            
            selected_pool_name = random.choices(pools, weights=weights, k=1)[0]
            # 5. 選択されたプールから最終的な話題を1つ選ぶ
            topic_pool = []
            if selected_pool_name == 'special' and special_topics:
                topic_pool = special_topics
            else: # 'general' またはフォールバック
                topic_pool = general_topics
            
            random_topic = random.choice(topic_pool) if topic_pool else None
    ...
    def request_speech(self, speaker, text, message_type, situation=None):
        ...
        # もしメッセージタイプが「タッチ反応」の場合、記憶評価関数をツールから除外する
        if message_type == "タッチ反応":
            print("[フィルタリング] タッチ反応のため、長期記憶の評価ツールを除外します。")
            if tools_config and tools_config[0].get('function_declarations'):
                # evaluate_and_store_memory と acknowledge_referenced_memories 以外の関数で新しいリストを作成
                original_functions = tools_config[0]['function_declarations']
                filtered_functions = [
                    func for func in original_functions 
                    if func['name'] not in ['evaluate_and_store_memory', 'acknowledge_referenced_memories']
                ]
                tools_config = [{"function_declarations": filtered_functions}]
        ...
        print(f"model_key\n{model_key}\n")
        print("──────────────────────────────────────────────────────────────────────────────")
        print(f"tools_config\n{tools_config}\n")
        print("──────────────────────────────────────────────────────────────────────────────")
        print(f"speaker.log_manager.get_formatted_log()\n{speaker.log_manager.get_formatted_log()}\n")
        print("──────────────────────────────────────────────────────────────────────────────")
    ...
    def start_event(self, character_controller, event_data, is_recollection=False):
        """イベントを開始し、アプリケーションをイベントモードに移行する。"""
        if self.is_event_running or self.is_processing_lock.locked():
            if not is_recollection: # 通常イベントの場合のみ警告
                 print("他のイベントまたは処理が実行中のため、新規イベントを開始できません。")
            else: # 回想は処理中でも上書きして開始できるようにする
                 print("処理を中断してイベント回想を開始します。")
                 if self.is_processing_lock.locked(): self.is_processing_lock.release()
            
        self.is_event_running = True
        
        # イベントの主役以外のキャラクターを待機モードにする
        for char in self.characters:
            if char and char != character_controller:
                char.enter_event_wait_mode()

        # 主役キャラクターにイベント実行を指示 (回想フラグを渡す)
        character_controller.start_event(event_data, is_recollection)

    def end_event(self, character_controller, event_data, is_recollection=False):
        """イベントを終了し、通常モードに復帰する。"""
        if not self.is_event_running:
            return
            
        print("イベントモードを終了し、通常モードに復帰します。")
        self.is_event_running = False
        
        # 全キャラクターを通常モードに戻す
        for char in self.characters:
            if char:
                char.exit_event_mode()

        # 回想モードでない場合のみ、イベントの完了を記録する
        if not is_recollection:
            self.event_manager.record_event_completion(
                character_controller.original_id,
                event_data.get("id")
            )

        # イベント終了後のコールバックが設定されていれば実行
        if callback := self._post_event_callback:
            self._post_event_callback = None # 一度実行したらクリア
            callback()
        else:
            # 通常のイベント終了時はクールタイムをリセット
            self.reset_cool_time()
