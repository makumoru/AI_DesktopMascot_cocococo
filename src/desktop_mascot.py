...
class DesktopMascot:
    ...
    def reload_config_and_services(self):
        ...
        try:
            ...
            # --- ▼▼▼ 修正箇所 ▼▼▼ ---
            # 6. 各エンジンにグローバル設定の再読み込みを指示 -> このループは不要でエラーの原因なので削除
            ...
            # 7. 各キャラクターに設定再読み込みを指示 (こちらが正しい処理)
            for char in self.characters:
                if char:
    ...
    def _check_models_worker(self):
        ...
        try:
            ...
            if not hasattr(self, '_periodic_check_started'):
                ...
                self.behavior_manager.schedule_periodic_checks()
    ...
    def _show_model_check_results(self, invalid_models, new_recommendations):
        ...
        if invalid_models:
            ...
            # メッセージボックスを表示
            messagebox.showerror("モデル設定エラー", message, parent=self.root)
        ...
        if new_recommendations:
            # カスタムダイアログを開く
            parent_window = self.char1.ui if self.char1 and self.char1.ui.winfo_exists() else self.root
