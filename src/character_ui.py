...
class CharacterUIGroup(MascotDNDWindow):
    ...
    def _place_window_initially(self):
        ...
        # config/position.iniから読み込んだ初期配置サイドに基づいて位置を決定
        ...
        # ジオメトリ確定後に、ハートとオーバーレイの位置を更新し、レイアウトを最終チェック
    ...
    def lift_with_heart(self):
        ...
        if self.overlay_window.winfo_viewable():
            self.overlay_window.lift(self.heart_window) # オーバーレイはハートの下、キャラクターの上
        # ★ここまで追加

    def move_with_heart(self, new_x, new_y):
        """キャラクターウィンドウとハート、オーバーレイウィンドウを一緒に動かす"""
    ...
    def update_geometry(self, force_x=None, force_y=None):
        self.update_idletasks()
        ...
        if force_x is not None and force_y is not None:
            ...
            # 上記以外のケース（ドラッグ完了後など）では、現在の位置を維持する
            try:
    ...
    def _update_heart_window_position(self):
        ...
        # キャラクターがどちら側にいるかの判定をインスタンス変数に保存
    ...
    def _update_overlay_window_position(self):
        ...
        # character_display_frameのスクリーン上の絶対座標とサイズを取得
    ...
    def apply_screen_effect(self, effect_type, color, method, duration_sec, callback):
        ...
        if effect_type == 'fade_out':
            ...
            if method == 'instant':
                ...
                # ★★★最終修正点★★★
                # 表示(deiconify)の後、重なり順変更(lift)の前にupdate()を挟む。
                # これにより、OSが古い見た目のままウィンドウをリフトしてしまうのを防ぎ、
                # 描画を強制的に完了させる。
                self.overlay_window.update()
                
                # 3. 見た目が確定してから、正しい重なり順に移動させる
                self.lift_with_heart()
    ...
    def show_favorability_tooltip(self, event):
        ...
        # Toplevelが実際に描画されるまで待機し、正確なサイズを取得
        ...
        # ハートウィンドウの絶対座標と、その中でのマウスカーソルの相対座標を取得
        ...
        # キャラクターの位置に応じてX座標を計算
        if self.is_on_left_side:
            # キャラクターが左側 -> ツールチップを右に展開
            x = base_x + mouse_x_in_heart + x_offset
        else:
            # キャラクターが右側 -> ツールチップを左に展開
    ...
    def show_exit_button(self, exit_callback):
        ...
        # event_proceed_buttonを「終了」ボタンとして再利用
        self.event_proceed_button.config(
            text="終了",
            command=exit_callback, # コマンドを最終シャットダウン処理に設定
            state="normal"
        )
        self.event_proceed_button.pack(side="right", padx=self.char_ctrl.mascot_app.padding_small, pady=self.char_ctrl.mascot_app.padding_small)
