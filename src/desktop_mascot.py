...
class DesktopMascot:
    ...
    def __init__(self, app_root_dir: str, current_version: str):
        ...
        if not self.config.has_option('UI', 'STARTUP_MODE'):
            print("config.iniにSTARTUP_MODEが見つからないため、デフォルト値を追加します...")
            try:
                with open('config.ini', 'r', encoding='utf-8-sig') as f:
                    lines = f.readlines()
                
                # 追記する設定行
                new_setting = "; 起動時のモード(fixed(通常),select(起動時に選択),random(ランダム選出))\nSTARTUP_MODE = fixed\n"
                ...
                if ui_section_index != -1:
                    # [UI]セクションが見つかった場合、そのセクションの末尾に設定を挿入
                    insert_pos = ui_section_index + 1
                    # 次のセクションが始まる前まで、またはファイルの最後までを探す
                    while insert_pos < len(lines) and not lines[insert_pos].strip().startswith('['):
                        insert_pos += 1
                    
                    # 挿入位置の直前の行が空行でなければ、見栄えのために空行を追加
                    if insert_pos > 0 and lines[insert_pos-1].strip() != "":
                        lines.insert(insert_pos, "\n")
                        insert_pos += 1 # 挿入位置を一つずらす

                    lines.insert(insert_pos, new_setting)
                else:
                    # [UI]セクションが見つからなかった場合、ファイルの末尾に追加
                    if lines and lines[-1].strip() != "":
                        lines.append("\n") # 見栄えのために空行を追加
                    lines.append("[UI]\n")
                    lines.append(new_setting)

                # 修正した内容でファイルを上書き
                with open('config.ini', 'w', encoding='utf-8') as f:
                    f.writelines(lines)

                # ファイルを更新したので、ConfigParserオブジェクトも最新の状態に再読み込みする
                self.config.read('config.ini', encoding='-utf-8-sig')
                print("config.iniにデフォルトのSTARTUP_MODE = fixed を追加しました。")

            except Exception as e:
                print(f"config.iniの更新中にエラーが発生しました: {e}")

        # 専用話題の抽選設定が存在しない場合に追記する
        if not self.config.has_option('UI', 'SPECIAL_TOPIC_BASE_WEIGHT'):
            print("config.iniに専用話題の抽選設定が見つからないため、デフォルト値を追加します...")
            try:
                with open('config.ini', 'r', encoding='utf-8-sig') as f:
                    lines = f.readlines()
                ...
                new_settings_block = (
                    ...
                    "; キャラクター固有の話題が1つ増えるごとに加算される確率 (%) (デフォルト: 1)\n"
                    "SPECIAL_TOPIC_PER_ITEM_WEIGHT = 1\n"
                    "; キャラクター固有の話題が選ばれる確率の上限値 (%) (デフォルト: 70)\n"
                    "SPECIAL_TOPIC_MAX_WEIGHT = 70\n"
                )
                ...
                insert_pos = len(lines) # デフォルトはファイルの末尾
                if ui_section_index != -1:
                    # [UI]セクションが見つかった場合、そのセクションの末尾に挿入
                    insert_pos = ui_section_index + 1
                    while insert_pos < len(lines) and not lines[insert_pos].strip().startswith('['):
                        insert_pos += 1
                
                lines.insert(insert_pos, new_settings_block)
                ...
                # 設定を再読み込み
                self.config.read('config.ini', encoding='-utf-8-sig')
                print("config.iniに専用話題の抽選設定を追加しました。")
