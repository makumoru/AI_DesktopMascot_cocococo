...
class EventRunner:
    ...
    """
    def __init__(self, character_controller, event_data, is_recollection=False):
        """
        EventRunnerを初期化します。
        ...
        Args:
            ...
            is_recollection (bool): 回想モードで実行するかどうかのフラグ。
        """
        self.char_ctrl = character_controller
        self.event_data = event_data
        self.sequence = event_data.get("sequence", [])
        self.current_step = -1
        self.is_recollection = is_recollection # 回想フラグを保持
        
        if self.is_recollection:
            print(f"** イベント '{self.event_data.get('id')}' を回想モードで開始します **")
    ...
    def _execute_current_command(self):
        ...
        # --- 回想モードでスキップするコマンドを定義 ---
        recollection_skip_commands = [
            "set_favorability",
            "add_long_term_memory",
            "set_flag",
            "change_persona",
            # ★画面効果は回想でも演出として実行するため、スキップ対象に含めない
        ]
        
        if self.is_recollection and command_type in recollection_skip_commands:
            print(f"[回想モード] コマンド '{command_type}' をスキップしました。")
            self.proceed() # 何もせず次のステップへ
            return
        ...
        # ★ここから追加
            ...
            # このコマンドは完了待機(wait)の設定に応じて内部でproceed()を呼ぶため、
            # ここでは呼び出しのみ行い、即座にreturnする。
            self.char_ctrl.execute_screen_effect(params)
            return
        # ★ここまで追加
            ...
            self.char_ctrl.execute_change_persona(params)
            # このコマンドも自動で次に進む
            self.proceed()
        elif command_type == "branch_on_flag":
            # 分岐は回想モードでも通常通り評価・実行される
