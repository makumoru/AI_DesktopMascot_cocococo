...
class EventManager:
    ...
    def _get_progress_file_path(self, character):
        ...
        return os.path.join(character.savedata_dir, "event_progress.ini")
    ...
    def load_character_data(self, character):
        ...
        with self.lock:
            self._ensure_progress_file_exists(character)
    ...
    def load_events_for_character(self, character):
        """指定されたキャラクターのイベント定義ファイルを読み込む。"""
        with self.lock:
            if not character: return
            
            char_id = character.original_id
            self.events_data[char_id] = {} # 既存のデータをクリア
            events_dir = os.path.join(character.character_dir, "events")
            
            if not os.path.isdir(events_dir):
                print(f"[{character.name}] eventsフォルダが見つかりません。")
                return
            ...
            print(f"[{character.name}] のイベントを{loaded_count}件読み込みました。")

    def unload_data_for_character(self, character):
        """指定されたキャラクターの全データをメモリから削除する。"""
        with self.lock:
            if not character: return
            char_id = character.original_id
            
            if char_id in self.events_data:
                del self.events_data[char_id]
            if char_id in self.progress_data_map:
                del self.progress_data_map[char_id]
            if char_id in self.flag_data_map:
                del self.flag_data_map[char_id]
            
            print(f"[{character.name}] のイベント関連データをアンロードしました。")

    def load_all_character_data(self):
        """現在画面にいる全キャラクターのデータをロードする"""
        with self.lock:
            self.events_data.clear()
            self.progress_data_map.clear()
            self.flag_data_map.clear()
            for char in self.app.characters:
                if char:
                    self.load_character_data(char)
                    self.load_events_for_character(char)
    ...
    def _parse_duration(self, duration_str: str) -> timedelta:
        """'24h', '7d', '30m' のような文字列をtimedeltaに変換する。"""
        if not isinstance(duration_str, str) or not duration_str:
            return timedelta.max # 不正な値は非常に長い時間として扱う
    ...
    def _check_single_condition(self, character, condition) -> bool:
        ...
        char_id = character.original_id
        char_flags = self.flag_data_map.get(char_id, {})
        progress_parser = self.progress_data_map.get(char_id)
        now = datetime.now()
        ...
        try:
            ...
            # --- イベント完了履歴 ---
            event_id_to_check = condition.get("event_id")
            if event_id_to_check:
                is_completed = progress_parser and progress_parser.has_section(event_id_to_check)
                if cond_type == "event_completed":
                    return is_completed
                if cond_type == "event_completed_after":
                    if not is_completed: return False
                    last_executed_str = progress_parser.get(event_id_to_check, "last_executed", fallback=None)
                    if not last_executed_str: return False
                    
                    last_executed_time = datetime.fromisoformat(last_executed_str)
                    required_duration = self._parse_duration(condition.get("duration", ""))
                    return now >= (last_executed_time + required_duration)
            ...
            if date_str:
                ...
                today = now.date()
                if cond_type == "date_equals": return today == target_date
                if cond_type == "date_after": return today >= target_date
                if cond_type == "date_before": return today <= target_date
            ...
            if time_str:
                ...
                if cond_type == "time_equals": return current_time.hour == target_time.hour and current_time.minute == target_time.minute
                if cond_type == "time_after": return current_time >= target_time
                if cond_type == "time_before": return current_time <= target_time

        except (ValueError, TypeError, KeyError) as e:
            # 型変換エラーやキーエラーは条件不一致とみなす
            print(f"イベント条件の評価中にエラーが発生しました: condition={condition}, error={e}")
            return False
        # どの条件タイプにも一致しなかった場合はFalse
        return False
    ...
    def get_completed_events_for_recollection(self, character):
        """指定されたキャラクターが完了したイベントのリストを返す。"""
        char_id = character.original_id
        progress_parser = self.progress_data_map.get(char_id)
        ...
        completed_list = []
        all_char_events = self.events_data.get(char_id, {})
        
        # progress_parserのセクション（完了済みイベントID）をループ
        for event_id in progress_parser.sections():
            if event_id == "Flags": continue # Flagsセクションは除外

            # 対応するイベント定義データを取得
            event_data = all_char_events.get(event_id)
            if event_data:
                # メニュー表示用の名前を取得 (なければIDをそのまま使う)
    ...
    def _is_event_ready_to_run(self, char, event_data) -> bool:
        ...
        event_id = event_data.get("id")
        trigger_groups = event_data.get("triggers")
