...
class ConversationLogManager:
    ...
    def __init__(self, character_save_dir_path, character_map, config: ConfigParser):
        """
        特定のキャラクターのログマネージャを初期化します。

        Args:
            character_save_dir_path (str): このログが属するキャラクターのセーブデータ用ディレクトリパス
            character_map (dict): キャラクターIDと名前を対応させる辞書。
            config (ConfigParser): アプリケーション全体の設定情報。
        """
        # キャラクターごとのログファイルパスを生成
        self.log_file_path = os.path.join(character_save_dir_path, self.LOG_FILE_NAME)
        self.character_map = character_map
        self.lock = threading.Lock()
        
        # configから設定値を読み込み、不正な場合はデフォルト値を使用
        try:
            self.log_history_limit = config.getint('UI', 'SHORT_TERM_MEMORY_LIMIT')
            if self.log_history_limit <= 0: # 念のため正の数かチェック
                print(f"警告: SHORT_TERM_MEMORY_LIMIT の値 ({self.log_history_limit}) が不正です。デフォルト値(30)を使用します。")
                self.log_history_limit = 30
        except (ValueError, NoSectionError, NoOptionError):
            self.log_history_limit = 30 # デフォルト値
